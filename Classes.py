    ...
    def get_effective(self, pokemon, op_pokemon):
        ...
        for element in element_att_strengths[op_pokemon.type]:
            if self.type == element:
                effectiveness *= 2
        # chicking if type is "not that effective"
...
class Pokemon:
    # xp_yield = (wild/owned * traded * base xp * feinted level)/7 * number in battle not feinted
...
# basic function for randomizing gender , possible refinement to reflect actual pokemon gender ratios
def get_gender():
...
# of the trainers current roster.
...
# example of defining each species of pokemons specific traits
...
class Bulbasaur(Pokemon):
    ...
    # {"HP": 45, "ATTACK": 49, "DEFENSE": 45, "SPEED": 45, "SPECIAL": 65} ivysaur = Pokemon("Ivysaur", ["GRASS,
    # POISON"], 2, get_gender(), {"HP":60,"ATTACK":62,"DEFENSE":63,"SPEED":60,"SPECIAL":80}, evolutions={"Venusaur":[
    # 32,3]}) venusaur = Pokemon("Venusaur", ["GRASS", "POISON"], 3, get_gender(), {"HP":80,"ATTACK":82,"DEFENSE":83,
    # "SPEED":80,"SPECIAL":100}) charmander = Pokemon("Charmander", ["FIRE"], 4, get_gender(),{"HP":39,"ATTACK":52,
    # "DEFENSE":43,"SPEED":65,"SPECIAL":50}, evolutions={"Charmeleon":[16,4]}) charmeleon = Pokemon("Venusaur",
    # ["FIRE"], 4, get_gender(), {"HP":58,"ATTACK":64,"DEFENSE":58,"SPEED":80,"SPECIAL":65})
